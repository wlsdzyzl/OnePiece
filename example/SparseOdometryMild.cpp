...
int main(int argc, char** argv)
{
    if(argc < 5)
    {
        std::cout<<"Usage: SparseOdometryMild [source_rgb] [source_depth] [target_rgb] [targe_depth]"<<std::endl;
        return 0;
    }
    ...
    bool is_success;
    timer.TICK("Spase Estimation Using Mild");
    auto tracking_result = rgbd_odometry.SparseTrackingMILD(source_rgb, target_rgb,source_depth, target_depth);
    timer.TOCK("Spase Estimation Using Mild");  
    timer.LogAll();      
