...
static inline std::shared_ptr<Shader> loadProgramFromFile(const std::string &shader_path, const std::string& vertex_shader_file, const std::string& fragment_shader_file)
{
    std::shared_ptr<Shader> program = std::make_shared<Shader>();

    program->AddShaderFromFile(pangolin::GlSlVertexShader, shader_path + "/" + vertex_shader_file, {}, {shader_path});
    program->AddShaderFromFile(pangolin::GlSlFragmentShader, shader_path + "/" + fragment_shader_file, {}, {shader_path});
    program->Link();
